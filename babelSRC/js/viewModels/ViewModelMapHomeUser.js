...
export default class ViewModelMapHomeUser {
  ...
  filterDriver(responseObserver, alDriver) {
    if (this.latitudeGPS !== null && this.longitudeGPS !== null) {
      let alResult = alDriver.filter(function (it) {
        return it.maxDist > this.calculateDist(this.latitudeGPS, this.longitudeGPS, it.latitude, it.longitude) || it.maxDist === 0;
      });
  ...
  makeVehiclesList(prices, distance, listVehicleObserver) {
    ...
    //Simple car
    this.listVehicles.push(new Vehicle("Auto básico", parseInt(prices.priceNormalCar * distance), 4, "Vehículo sencillo con alrededor de 4 capacidades, ideal para obtener mejores precios"));

    //Comfort
    this.listVehicles.push(new Vehicle("Auto de confort", parseInt(prices.priceComfortCar * distance), 4, "Vehículo muy cómodo con alrededor de 4 capacidades y aire acondicionado"));

    //Familiar
    this.listVehicles.push(new Vehicle("Auto familiar", parseInt(prices.priceFamiliarCar * distance), 8, "Vehículo cómodo con alrededor de 8 capacidades, ideal para el viaje en familia"));

    //Tricycle
    this.listVehicles.push(new Vehicle("Triciclo", parseInt(prices.priceTricycle * distance), 2, "Vehículo triciclo con alrededor de 2 capacidades, ideal para viajes cortos"));

    //Bicitaxi
    this.listVehicles.push(new Vehicle("Bicitaxi", parseInt(prices.priceBiciTaxi * distance), 2, "Vehículo con solo 2 capacidades, ideal para viajes cortos y cómodos"));

    //Motorcycle
    this.listVehicles.push(new Vehicle("Motor", parseInt(prices.priceMotorcycle * distance), 1, "Vehículo con solo 1 capacidad, ideal para viajes rápidos y sin mucho equipaje"));
    listVehicleObserver(this.listVehicles);
  }
  addTrip(stateObserver, prices, phone, typeCar) {
    this.tripsDataSource.addTrip(stateObserver, "no", UserAccountShared.getUserEmail(), parseInt(prices), this.lastDistance, this.makeDate(), this.latitudeDestiny, this.longitudeDestiny, this.latitudeOrigin, this.longitudeOrigin, phone, typeCar);
  }
  makeDate() {
    let allDate;
