...
export default class ViewModelMapHomeDriver {
  ...
  filterDriver(listDriverObserver, alDriver) {
    if (this.latitudeGps && this.longitudeGps) {
      let alResult = alDriver.filter(it => 20 > this.calculateDist(this.latitudeGps, this.longitudeGps, it.latitude, it.longitude));
  ...
  updateDriverLocationStop(stateDriverLocationStop) {
    if (DriverAccountShared.getDriverEmail()) {
      this.driverDataSource.updateDriverLocation(stateDriverLocationStop, DriverAccountShared.getDriverEmail(), 0.0, 0.0);
    }
  }
  ...
  getAllTrips(stateAllTripsObserver, responseAllTripObserver) {
    if (DriverAccountShared.getDriverEmail()) {
      this.tripsDataSource.getTrips(stateAllTripsObserver, responseAllTripObserver, "En espera", DriverAccountShared.getDriverEmail());
    }
  }
