  ...
  updateRecyclerInfo(json_it) {
    if (json_it.length > 0) {
      ...
      for (let f = 0; f < json_it.length; f++) {
        ...
        pTypeCar.innerHTML = "Veh&iacute;culo: " + json_it[f].type;
        let pPriceCar = document.createElement("p");
        pPriceCar.setAttribute("class", "card-taxi-p");
        pPriceCar.innerHTML = "Precio: " + json_it[f].price + " CUP";
      ...
      if (this.viewModelMapHome.longitudeGPS) {
        if (working) {
          ...
          while (this.viewModelMapHome.activateAnimation) {
            ...
            this.markerAnimation = new mapboxgl.Marker(img).setLngLat([this.viewModelMapHome.longitudeGPS, this.viewModelMapHome.latitudeGPS]).addTo(this.map);
    ...
    if (this.viewModelMapHome.listDrivers) {
      for (let f = 0; f < this.viewModelMapHome.listDrivers.length; f++) {
        if (this.viewModelMapHome.listDrivers[f].longitude !== 0.0 && this.viewModelMapHome.listDrivers[f].latitude !== 0.0) {
          this.addAnnotationDrivers(new Point(this.viewModelMapHome.listDrivers[f].longitude, this.viewModelMapHome.listDrivers[f].latitude), this.getDriverMarkerImg(this.viewModelMapHome.listDrivers[f].typeCar));
        }
      }
    }
  }
